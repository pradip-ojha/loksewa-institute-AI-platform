...
class TestValidateSection:
    ...
    def test_model_tick_point_kept(self):
        mask = g.build_ink_mask(_page_with_ink())
        out = g._validate_section(self._sec(), 1000, 1400, ink_mask=mask)
        assert out["source"] == "model"

    def test_bad_tick_point_moves_to_margin(self):
        out = g._validate_section(self._sec(tick_point=[350, 220]), 1000, 1400)
        assert out["source"] == "margin"
        assert out["tick_point"][0] < 100  # left of the evidence box start
